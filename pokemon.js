...
const namesPoison = pokémon.filter(p => p.types.length == 1 && p.types[0] === "poison").map(p => p.name);
console.log(namesPoison);

const otherType = pokémon.filter(p => p.types[1] === "flying").map(p => p.types[0])
console.log(otherType);

const normalReduce = pokémon.filter(p => p.types[0] === "normal").map(p => p = 1).reduce((a, b) => a + b)
